...
h2o.download_pojo <- function(model, path=NULL, getjar=NULL, get_jar=TRUE) {
  ...
  if (get_jar) {
    ...
    writeBin(getBinaryURL(.h2o.doSafeGET(urlSuffix = urlSuffix, binary = TRUE)), jar.path, useBytes = TRUE)
  }
...
h2o.download_mojo <- function(model, path=getwd(), get_genmodel_jar=FALSE) {
  ...
  writeBin(getBinaryURL(.h2o.doSafeGET(urlSuffix = urlSuffix, binary = TRUE)), mojo.path, useBytes = TRUE)
